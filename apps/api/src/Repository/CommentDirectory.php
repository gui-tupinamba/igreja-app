    ...
    public function list(
        ...
        $scope = $administrative
            ? "EXISTS (
                ...
                WHERE a.id = :actor_id
                  AND a.status = 'ACTIVE'
                  AND a.role IN ('ADMIN','PASTOR')
            )"
            : ContentReadScope::predicate(
                'posts',
                'p',
            );

        $status = $administrative
            ? "c.status IN ('VISIBLE','HIDDEN')"
            : "c.status = 'VISIBLE'";

        /*
         * Além dos dados do comentário,
         * agora buscamos também o nome
