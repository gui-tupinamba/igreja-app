...
final readonly class ScheduleDirectory
{
    ...
    public function list(int $actorId, int $page, int $limit, array $filters = [], bool $administrative = false): array
    {
        ...
        $scope = $administrative ? self::MANAGE_SCOPE : ContentReadScope::predicate('ministry_schedules', 'p');
        ...
        $rows = $this->db->fetchAllAssociative("WITH authorized AS (
                SELECT p.id, p.created_at, p.starts_at FROM ministry_schedules p WHERE ($scope)
            ...
            LEFT JOIN page_keys k ON TRUE LEFT JOIN ministry_schedules p ON p.id = k.id
            ORDER BY ".($administrative ? 'k.created_at DESC, k.id DESC' : 'k.starts_at ASC, k.id ASC'), $parameters, $types);
        $items = [];
        foreach ($rows as $row) { if ($row['id'] !== null) { $items[] = ScheduleView::data($row); } }
    ...
    public function detail(int $actorId, int $id, bool $administrative = false): ?array
    {
        if ($administrative) { $this->requireManager($actorId); }
        $scope = $administrative ? self::MANAGE_SCOPE : ContentReadScope::predicate('ministry_schedules', 'p');
        $row = $this->db->fetchAssociative("SELECT p.* FROM ministry_schedules p WHERE p.id = :id AND ($scope)", ['id' => $id, 'actor_id' => $actorId]);
        return $row === false ? null : ScheduleView::data($row);
    }
