    ...
    <Screen refreshControl={undefined}>
      <View style={styles.headingRow}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [styles.bell, pressed && { opacity: 0.65 }]}
        >
          <Ionicons
      ...
      <View style={styles.hero}>
        <Text style={styles.heroEyebrow}>A IGREJA ACONTECE COM VOCÊ</Text>
        <Text style={styles.heroTitle}>
          Encontre seu lugar. Faça parte do próximo encontro.
        </Text>
        <Pressable
          onPress={() => router.push("/(app)/(tabs)/agenda")}
          style={styles.heroButton}
        >
          <Text style={styles.heroButtonText}>Ver nossa agenda</Text>
        </Pressable>
      </View>
      {loading ? <Loading /> : null}
      {posts.error ? (
        <ErrorState error={posts.error} retry={posts.reload} />
      ) : null}
      {!posts.loading && !posts.error ? (
        <>
          <Text style={styles.section}>Na vida da comunidade</Text>
          {posts.data?.items.length ? (
            ...
            <Empty>As próximas publicações aparecerão aqui.</Empty>
          )}
        </>
      ) : null}
      {agenda.error ? (
        <ErrorState error={agenda.error} retry={agenda.reload} />
      ) : null}
              ...
              >
                <Card>
                  ...
                  <Text numberOfLines={2} style={styles.body}>
                    {item.description || "Confira os detalhes deste encontro."}
                  </Text>
                </Card>
              </Pressable>
            ))
...
const styles = StyleSheet.create({
  ...
  heroButtonText: { color: colors.greenDark, fontWeight: "800" },
  section: {
    color: colors.ink,
    fontSize: 20,
    fontWeight: "800",
  ...
  date: { color: colors.green, fontWeight: "700", fontSize: 13 },
});
